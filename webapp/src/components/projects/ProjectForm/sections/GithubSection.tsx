      ...
      {isOpen && (
        <div className={styles.sectionContent}>
          <p className={styles.sectionDescription}>
            在 GitHub 仓库中搜索与目标域名相关的泄露密钥、API Key 与凭据，识别可能导致系统与服务被未授权访问的敏感信息泄露。
          </p>
          ...
          {hasGithubToken && (
            <>
              <div className={styles.subSection}>
                ...
                <div className={styles.toggleRow}>
                  <div>
                    <span className={styles.toggleLabel}>扫描 Commits</span>
                    <p className={styles.toggleDescription}>在提交历史中搜索泄露密钥</p>
                    <TimeEstimate estimate="最耗时的操作——关闭可节省 50%+ 时间" />
                  </div>
                  <Toggle
              ...
              {data.githubScanCommits && (
                <div className={styles.fieldGroup}>
                  <label className={styles.fieldLabel}>最大扫描提交数</label>
                  <input
                    type="number"
                    className="textInput"
                    ...
                    max={1000}
                  />
                  <span className={styles.fieldHint}>每个仓库最多扫描的提交数量</span>
                  <TimeEstimate estimate="线性增长：100 = 默认；1000 ≈ 慢 10 倍" />
                </div>
              )}

              <div className={styles.toggleRow}>
                <div>
                  <span className={styles.toggleLabel}>输出为 JSON</span>
