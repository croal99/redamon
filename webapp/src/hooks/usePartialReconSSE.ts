...
interface UsePartialReconSSEOptions {
...
interface UsePartialReconSSEReturn {
  logs: ReconLogEvent[]
  isConnected: boolean
  error: string | null
  clearLogs: () => void
...
export function usePartialReconSSE({
  projectId,
  enabled,
  onLog,
  onPhaseChange,
  onComplete,
  onError,
}: UsePartialReconSSEOptions): UsePartialReconSSEReturn {
  ...
  const connect = useCallback(() => {
    ...
    const eventSource = new EventSource(`/api/recon/${projectId}/partial/logs`)
    ...
    eventSource.addEventListener('error', (event) => {
      try {
        ...
        console.error('Error parsing partial recon SSE error event:', err)
      }
    })
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        console.error('Error parsing partial recon SSE message:', err)
      }
    }
...
export default usePartialReconSSE
