...
export default function ProjectsPage() {
  ...
  const handleDeleteProject = async (projectId: string) => {
    if (await dangerConfirm('确定要删除此项目吗？该操作不可逆。')) {
  ...
  const handleDeleteUser = async () => {
    ...
    const warning = projectCount > 0
      ? `这将永久删除用户 "${selectedUser?.name}" 及其关联的 ${projectCount} 个项目。此操作无法撤销。`
      : `确定要删除用户 "${selectedUser?.name}" 吗？该操作不可逆。`
      ...
      {showUserModal && (
        <div className={styles.modalOverlay} onClick={() => setShowUserModal(false)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <h2 className={styles.modalTitle}>创建新用户</h2>
            <form onSubmit={handleCreateUser}>
              <div className="formGroup">
                <label className="formLabel formLabelRequired">姓名</label>
                <input
                  type="text"
                  className="textInput"
                  value={newUserName}
