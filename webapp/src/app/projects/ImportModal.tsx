        ...
        {status === 'success' && result && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
            ...
            <div style={{
              ...
              <span>会话数: {result.stats.conversations}</span>
              <span>消息数: {result.stats.messages}</span>
              <span>修复方案: {result.stats.remediations}</span>
              <span>报告数: {result.stats.reports}</span>
              <span>图谱节点: {result.stats.neo4jNodes}</span>
              <span>节点关系: {result.stats.neo4jRelationships}</span>
              <span>产物数量: {result.stats.artifacts}</span>
            </div>
            <div className={styles.modalActions}>
